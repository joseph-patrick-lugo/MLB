...
def insert_teams():
    ...
    with engine.begin() as conn:
        ...
        league_map = {row['name']: row['id'] for row in conn.execute(text("SELECT id, name FROM league"))}
        division_map = {row['name']: row['id'] for row in conn.execute(text("SELECT id, name FROM division"))}
